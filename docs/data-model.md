...
| `clientMessageAdmissions` | `{ version: 1, entries: Record<string, { fingerprint: string, status: "pending" \| "committed" \| "legacy_unverifiable" }>, legacyOverflow?: boolean }?` | Durable bounded idempotency ledger keyed by normalized client message ID. IDs are capped at 256 characters, the ledger stores at most 4096 entries, and replay checks do not advance agent activity timestamps. New admissions carry the SHA-256 fingerprint of the normalized provider prompt plus `pending`/`committed` status. Timeline-derived legacy entries use the fail-closed `legacy_unverifiable` state when the original structured payload cannot be reconstructed; callers must retry those deliveries with a new message ID. |
