...
|    20 | [#1703](https://github.com/getpaseo/paseo/pull/1703) OpenCode cwd scoping                                                                                     | P1       | Rebase current reviewed head — **done (through commit fb2393b3d)**                           |
|    21 | [#485](https://github.com/getpaseo/paseo/pull/485) remaining OpenCode fixes                                                                                   | P1       | Extract remaining behavior — **done (through commit 0690b86b2)**                             |
...
**Port notes:** Cherry-picked both commits onto `feature/p0-integration-build`. The
second commit auto-merged except a content conflict in
`packages/server/src/server/agent/tools/paseo-tools.ts`, resolved by combining the
sender-envelope injection with the existing `dispatchResult` capture and
`ownBackgroundAgentRunStart` path from prior integration backports. A focused
technical review then found two behaviors that exist identically in the reviewed
upstream PR but conflict with the queue's deterministic-projection criterion; both
were fixed in a follow-up commit (5c922e0c6): (1) `primeTimelineFromLegacyProviderHistory`
now broadcasts the projected item like its `forceHydrate` sibling instead of the raw
stream event; (2) the agent-message projection regex is greedy so a prompt embedding
the closing tag is not truncated for display (the receiving model still sees the
unescaped original; general body XML-escaping was rejected because it would alter the
delivered prompt).
